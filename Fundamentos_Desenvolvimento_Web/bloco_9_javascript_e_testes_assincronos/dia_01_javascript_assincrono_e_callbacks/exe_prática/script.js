...
// tempo de execução 'igual', assim que for efetuado a leitura de forma padrão. Vai sendo executado assim que uma acabar, entra a nova. 'igual' pois pra nossa visualização, não existe delay.
...
// A -> C -> B
// Existe timeout em duas chamadas, em B e C. Sendo B um tempo maior de timeout.
// ter um timeout maior, ou seja, ter o valor do segundo parâmetro da função timeout maior, tem um tempo maior em 'espera', um tempo que vai demorar mais pra ser executado.
...
const sendMarsTemperature = (callback) => {
	setTimeout(() => {
		callback(getMarsTemperature().toFixed(1));
	}, messageDelay());
};
